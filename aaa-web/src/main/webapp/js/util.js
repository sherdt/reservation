var Util = (new function($) {
	...
	this.formatDateTime = function(unixTime) {
		...
		var m = dt.getMinutes();
		var s = dt.getSeconds();
		return dt.toDateString() + ' ' + (h < 10 ? '0' : '') + h + ':' + (m < 10 ? '0' : '') + m + ':'
				+ (s < 10 ? '0' : '') + s;
	};
	...
	this.formatNZBName = function(NZBName) {
		return NZBName.replace(/\./g, ' ').replace(/_/g, ' ');
	};
...
var TabDialog = (new function($) {
	...
	function maximize(options) {
		...
		if (options.mini) {
			...
			scroll.css('min-height', dialog.height() - header.outerHeight() - footer.outerHeight()
					- scrollheader.height() - bodyPadding * 2);
		}
	}
	;
	...
	function switchTab(fromTab, toTab, duration, options) {
		...
		var dialogStyleFS = null, bodyStyleFS = null, footerStyleFS = null;
		...
		if (options.fullscreen && options.back) {
			...
			// restore non-fullscreen state to calculate proper destination
			// sizes
			dialog.restoreTab();
		}
		...
		var newBodyHeight = fullscreen ? windowHeight - header.outerHeight() - footer.outerHeight() - dialogMargin * 2
				- bodyPadding * 2 : body.height();
		...
		fromTab.animate({
			left : sign * -((options.back ? newTabWidth : oldTabWidth) + bodyPadding * 2),
			height : newBodyHeight + bodyPadding
		}, duration);
		toTab.animate({
			left : leftPos,
			height : newBodyHeight + bodyPadding
...
var RPC = (new function($) {
	...
	this.call = function(method, params, completed_callback, failure_callback, timeout) {
		...
		// window.btoa('myusername:mypassword'));
