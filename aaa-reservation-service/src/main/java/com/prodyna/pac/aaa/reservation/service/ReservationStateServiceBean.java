...
public class ReservationStateServiceBean implements ReservationStateService {
	...
	@Override
	public ReservationState readReservationState(final String name) throws EntityNotFoundException {
		final ReservationState reservationState = this.entityManager.find(ReservationState.class, name);
		if (reservationState == null) {
			throw new EntityNotFoundException("Reservation state could not be found for given username [" + name + "]");
		}
